    ...
    <AisInstantSearchSsr>
      ...
      <div class="bg-white mb-16">
        <div class="container py-10 pb-16">
          ...
          <h1 class="text-4xl text-gray-700 font-medium leading-tight mb-6">Search</h1>
          <client-only>
            <AisPoweredBy />
          </client-only>
          </div>
        ...
        <AisStateResults>
          <div slot-scope="{query}">
             <template v-if="query.length">
                <AisStats>
                  <h1 class="text-xl text-gray-600 font-medium mb-6" slot-scope="{nbHits}">Snippets ({{nbHits}})</h1>
                </AisStats>
              <AisHits>
  ...
  export default {
    ...
    async asyncData({app}) {
        try {
          ...
          return instantsearch.findResultsState({hitsPerPage:20,filters:'data.is_public:true'})
            .then(()=>{
              return {
